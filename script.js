...
function moneyConverter (valNum) {
    document.getElementById("outputrub").innerHTML=valNum*77
}

let myNodeList = document.getElementsByTagName('li')
// Строчку let i мы просто убрали к чертям, так как i уже создана выше

for(i=0; i< myNodeList.length; i++) { // Тут i просто используется, всё чётко
...
function newElement() {
    ...
    if(inputValue === '') {
        ...
        // Добавили создание крестика для нового таска
        let span = document.createElement("SPAN")
        let txt = document.createTextNode("\u00D7")
        span.className = "close"
        span.appendChild(txt)
        ...
        // Научили новый крестик удалять таск при клике
        span.onclick = function () {
            let div = this.parentElement
            div.style.display = "none"
        }
